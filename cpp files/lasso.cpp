...
void Lasso::loopit(int radius) {
  if(lasso_looped) { return; } // Already looped
  lasso_loop.reset(getXPos(), getYPos(), radius);
...
void Lasso::check_for_coin(Coin *coinPtr, int radius) {
...
int Lasso::check_for_mag(Mag *magPtr) {
  double lasso_x = getXPos();
  double lasso_y = getYPos();
